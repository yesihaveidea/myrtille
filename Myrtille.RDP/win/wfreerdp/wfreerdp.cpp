...
process_params(wfInfo * wfi, int argc, char ** argv, int * pindex)
{
	...
	for(i = 1; i < argc; i++)
		#pragma region Myrtille
		if (strcmp("--no-window", argv[i]) == 0)
		{
			settings->show_window = 0;
			settings->show_console = 0;
		}
		#pragma endregion
		else if (strcmp("--no-console", argv[i]) == 0)
		{
			settings->show_console = 0;
		}
		...
		#pragma region Myrtille
		else if (strcmp("--debug-log-process", argv[i]) == 0)
		{
			...
			if (logDirectoryPath != "")
			{
				// create the log file
				std::stringstream ss;
				ss << logDirectoryPath << "\\FreeRDP.wfreerdp." << GetCurrentProcessId() << ".log";
				std::string s = ss.str();
	...
	while (*pindex < argc)
	{
		...
		else if (strcmp("-i", argv[*pindex]) == 0)
		{
			*pindex = *pindex + 1;
			if (*pindex == argc)
			{
				printf("missing remote session id\n");
				return 1;
			}
			settings->remotesession_id = atoi(argv[*pindex]);
		}
		#pragma endregion
#ifndef DISABLE_TLS
...
run_wfreerdp(wfInfo * wfi)
{
	...
	#pragma region Myrtille
	// no keyboard management when not using window (no user interaction)
	...
	#pragma region Myrtille
	// create myrtille pipes
	if (wfi->settings->remotesession_id != 0)
	{
		DWORD connect_pipes = wf_connect_pipes(wfi);
		if (connect_pipes != 0)
		{
			printf("run_wfreerdp: wf_connect_pipes failed with error %d\n", connect_pipes);
			return 1;
		}
		else
		{
			printf("run_wfreerdp: wf_connect_pipes succeeded for remote session %i\n", wfi->settings->remotesession_id);
		}
	}
	#pragma endregion
...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	#pragma region Myrtille
	wfi->image_encoding = IMAGE_ENCODING_JPEG;
	wfi->image_quality = IMAGE_QUALITY_HIGH;
	wfi->gdiplusToken = gdiplusToken;
	wfi->pngClsid = pngClsid;
	wfi->jpgClsid = jpgClsid;
	wfi->encoderParameters = encoderParameters;
	wfi->webpConfig = webpConfig;
	#pragma endregion
	...
	else
	{
		...
		if (g_thread_count > 0)
			...
			printf("Failed to start wfreerdp\n");
	}
